    ...
    class Program
    {
        static void Main(string[] args)
        {
...

            {
                ...
                //TODO
                List<string> str = new List<string> { "1qwerty", "cqwertyc", "1qwerty", "cqwe", "c" };
                var res = str.Select(x => x[0]);
                //string res = str.Aggregate((x, y) => x[0].ToString() + y[0].ToString());
...

            {
                ...
                //TODO
                int k = 3;
                List<string> str = new List<string> { "32K23A", "33423EW", "23F", "3R7"};
                var res = str.Where(x => x.Length == k && char.IsDigit(x[x.Length - 1]));
...

            {
                ...
                //TODO
                int d = 3, k = 4;
                IEnumerable<int> a = new int[] { 12, 88, 1, 3, 5, 4, 6, 6, 2, 5, 8, 9, 0, 90 };
                //var res = a.Where(a.TakeWhile(x => x < d).Except(a.Skip(k))).Distinct().OrderByDescending(i => i));
...

            {
                ...
                //TODO
                List<string> str = new List<string> { "32K23A", "33423EW", "23F", "3R7" };
                var res = ((str.Where(x => x.Length % 2 != 0).Select(x => x[0])) ?? str.Select(x => x[x.Length - 1]))
                    .OrderByDescending(x => x.GetHashCode());
            }

            {
                ...
                var res = a.Where(x => x > k1).Concat(b.Where(x => x < k2)).OrderBy(i => i);
            }
            ...
            }
            {
                ...
                //TODO
                List<string> a = new List<string> { "12K23A", "13423EW", "13F", "1R7" };
                List<string> b = new List<string> { "22K23A", "23423EW", "23F", "2R7" };
                //var res = a.Join(b, x => x.Length, y => y.Length, (x, y) => x.ToString() + ": " + y.ToString()).OrderBy((x,y)=>(x == y));
                var res = a.Join(b, x => x.Length, y => y.Length, (x, y)=> new {FirstValue = x, SecondValue = y})
                    .OrderBy(x => x.FirstValue).ThenBy(x => x.SecondValue);
                
                Console.WriteLine(new string('-',100));
                foreach (var VARIABLE in res)
                {
                    Console.WriteLine("{0}: {1}", VARIABLE.FirstValue, VARIABLE.SecondValue);
                }
            }

            {
...

                {
                    ...
                    { new Applicant(11, 2015, "Serko"), 
                        new Applicant(11, 2014, "Borisov"),
                        new Applicant(12, 2011, "Vrest"),
                        new Applicant(12, 2011, "Zaritskyi"),
                        new Applicant(11, 2014, "Borisov"),
                        new Applicant(12, 2012, "Vrest"),
                        new Applicant(12, 2011, "Zaritskyi")
                    };

                    var result = applicants
                        .GroupBy(x => x.YearOfEntry)
                        .OrderBy(x => x.Count())
                        .ThenBy(x => x.Key);
                    ...
                    foreach (var VARIABLE in result)
                    {
                        ...
                        foreach (var school in VARIABLE)
                        {
                            if (school.YearOfEntry == VARIABLE.Key)
                            {
                                numberOfSchool++;
                            }
                        }

                        Console.WriteLine("Число школ: {0}, год: {1}", numberOfSchool, VARIABLE.Key);
                    }
                }
            }
        }
